...
const groupByReleaseId = <T extends { release_id: number }>(items: readonly T[]) => {
...
const buildPetsForCharacter = (characterId: number): CharacterIndexData["relationships"]["pets"] => {
  ...
  return petOwnerships
    .map((ownership) => {
      const pet = petById.get(ownership.pet_id as number) as any;
      if (!pet) return null;
...
const buildReleasesForCharacter = (characterId: number) => {
  ...
  const releases = Array.from(releaseIds)
    .map((releaseId) => {
      const release = releaseById.get(releaseId) as any;
