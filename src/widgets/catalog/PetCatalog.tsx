...
import { usePetsList } from '@/shared/api/hooks';
...
const PetCatalog: React.FC = () => {
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('name');
  const [currentPage, setCurrentPage] = useState(1);
  const { data: pets = [], isPending } = usePetsList();

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    const base = q
      ? pets.filter((item) => {
          const haystack = `${item.name} ${item.species} ${item.ownerName ?? ''}`.toLowerCase();
        ...
        {isPending && filtered.length === 0 ? (
          <Grid container spacing={{ xs: 1.5, sm: 2, md: 2.5 }}>
            {Array.from({ length: 12 }).map((_, i) => (
              <Grid size={{ xs: 6, sm: 4, md: 3, lg: 3 }} key={i}>
                <CatalogCardSkeleton imagePaddingTop="150%" />
              </Grid>
            ))}
          </Grid>
        ) : (
          <Grid container spacing={{ xs: 1.5, sm: 2, md: 2.5 }}>
            {paginated.map((pet, index) => (
              <Grid size={{ xs: 6, sm: 4, md: 3, lg: 3 }} key={pet.id}>
                <PetCardCatalog pet={pet} priority={index < 4} />
              </Grid>
            ))}
          </Grid>
        )}
